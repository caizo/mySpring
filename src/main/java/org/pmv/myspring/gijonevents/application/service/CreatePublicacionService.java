...
public class CreatePublicacionService implements CreatePublicacionUseCase {
    ...
    @Override
    public CreatePublicacionResult create(CreatePublicacionCommand command) {

        List<String> imagenesGuardadas = new ArrayList<>();

        try {
            if (command.getImagenes() != null) {
                for (ImageInput imagen : command.getImagenes()) {

                    String imagenUrl = imageStorage.save(
                            imagen.getInputStream(),
                            imagen.getFileName(),
                            imagen.getContentType()
                    );

                    imagenesGuardadas.add(imagenUrl);
                }
            }
            ...
            eliminarImagenes(imagenesGuardadas);
            throw exception;
        }
    }
