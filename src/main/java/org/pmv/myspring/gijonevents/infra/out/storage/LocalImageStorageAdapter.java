...
public class LocalImageStorageAdapter implements ImageStoragePort {
    ...
    public LocalImageStorageAdapter(@Value("${app.storage.upload-dir}") String uploadDir) {
        ...
        try {
            ...
            throw new IllegalStateException("No se pudo crear el directorio de imágenes", exception);
        }
    }
    ...
    public void delete(String imageUrl) {
        ...
        Path file = uploadDirectory.resolve(imageUrl).normalize();

        if (!file.startsWith(uploadDirectory)) {
            throw new IllegalArgumentException("Ruta de imagen no válida");
        }
